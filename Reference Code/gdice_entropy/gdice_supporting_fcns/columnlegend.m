function [legend_h,object_h,plot_h,text_strings] = columnlegend(plotHandles,numcolumns, str, varargin)
%
...
%create the legend
[legend_h,object_h,plot_h,text_strings] = legend(plotHandles,str,'location','southoutside','fontsize',11.5);
loci_orig = get(gca, 'position');
...
for i=1:numlines,
    ...
    %realign the labels
    set(object_h(linenum), 'ydata', [shift_y+(height-(position-1)*sheight) shift_y+(height-(position-1)*sheight)]);
    set(object_h(linenum), 'xdata', [col/numcolumns+spacer col/numcolumns+spacer+line_width]);
    
    set(object_h(linenum+1), 'ydata', [shift_y+height-(position-1)*sheight shift_y+height-(position-1)*sheight]);
    set(object_h(linenum+1), 'xdata', [col/numcolumns+spacer*3.5 col/numcolumns+spacer*3.5]);
    
    set(object_h(labelnum), 'position', [col/numcolumns+spacer*2+line_width shift_y++height-(position-1)*sheight]);
...
if boxon,
    ...
    pos(2)= pos(2) + orgHeight-pos(4) - pos(4)*0.05 +0.04;
    pos(1) = pos(1)+pos(1)*0.05 - 0.005;
    annotation('rectangle',pos)
end

set(gca, 'position',loci_orig)
